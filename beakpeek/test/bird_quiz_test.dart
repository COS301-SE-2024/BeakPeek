...
void main() {
    ...
    test('returns a list of birds if the http call completes successfully', () async {
      ...
      when(client.get(Uri.parse('http://10.0.2.2:5000/api/Bird/')))
          .thenAnswer((_) async => http.Response(
              '[{"bird": {"ref": 1, "common_group": "Heron", "common_species": "Black-headed Heron", "genus": "Ardea", "species": "melanocephala", "full_Protocol_RR": 10.0, "full_Protocol_Number": 1, "latest_FP": "2022-01-01T00:00:00Z"}, "pentad": {"pentad_Allocation": "12345", "pentad_Longitude": 30.0, "pentad_Latitude": -25.0, "province": {"id": 1, "name": "TestProvince"}, "total_Cards": 1}, "jan": 1.0, "feb": 1.0, "mar": 1.0, "apr": 1.0, "may": 1.0, "jun": 1.0, "jul": 1.0, "aug": 1.0, "sep": 1.0, "oct": 1.0, "nov": 1.0, "dec": 1.0, "total_Records": 12, "reportingRate": 10.0}]', 200));
  ...
  group('getImages', () {
    test('returns a list of image URLs if the http call completes successfully', () async {
      final client = MockClient();
      final bird = Bird(
        id: 1,
        pentad: Pentad(
      ...
      when(client.get(Uri.parse('http://10.0.2.2:5000/api/BirdInfo/Black-headed Heron Heron')))
          .thenAnswer((_) async => http.Response(
              '{"images": [{"url": "https://example.com/image1.jpg"}, {"url": "https://example.com/image2.jpg"}]}', 200));
